---
name: framer-project-rw2FB3HU0jEe9zIusTs8
description: "Project-scoped Framer skill for project rw2FB3HU0jEe9zIusTs8. Covers project reads, edits, change review, publishing, image sourcing, component operations, and canvas editing. Very important: never load this skill without having already read the `framer` skill and without having already run `session new`, which will dynamically update this skill."
allowed-tools: ["Bash(npx @framer/agent:*)", "Bash(npx @framer/agent@latest:*)", "Read(C:\Users\mayan\AppData\Local\Temp\framer/*)", "Write(C:\Users\mayan\AppData\Local\Temp\framer/*)"]
---

## Project Scope

- Project ID: rw2FB3HU0jEe9zIusTs8
- Generated At: 2026-06-28T12:32:29.194Z
...
Only after checking docs, use your write tool to write your code to a unique file under `C:\Users\mayan\AppData\Local\Temp\framer/`, then execute it with `-f`. Do not create code files with shell heredocs or `cat`. Name each file `<sessionId>-<short-summary>.js` where `<short-summary>` is a brief kebab-case description (e.g., `1-read-collections.js`, `1-add-team-member.js`).

```bash
npx @framer/agent@latest exec -s 1 -f C:\Users\mayan\AppData\Local\Temp\framer/1-read-collections.js
```
...
```js
// C:\Users\mayan\AppData\Local\Temp\framer/1-get-collections.js
state.collections = await framer.getCollections();
```

```bash
npx @framer/agent@latest exec -s 1 -f C:\Users\mayan\AppData\Local\Temp\framer/1-get-collections.js
```

```js
// C:\Users\mayan\AppData\Local\Temp\framer/1-get-team-items.js — reuse from state
state.teamItems = await state.collections.find(c => c.name === 'Team').getItems();
console.log(state.teamItems.length);
```

```bash
npx @framer/agent@latest exec -s 1 -f C:\Users\mayan\AppData\Local\Temp\framer/1-get-team-items.js
```
...
Prefer using your write tool to write code to a unique file under `C:\Users\mayan\AppData\Local\Temp\framer/` and executing it with `-f`. Do not create code files with shell heredocs or `cat`:

```bash
npx @framer/agent@latest exec -s <sessionId> -f C:\Users\mayan\AppData\Local\Temp\framer/<sessionId>-<short-summary>.js
```
...
state.localCodePath = "C:\Users\mayan\AppData\Local\Temp\framer/Badge.tsx";
...
<project-fonts>["Inter","Syne","Panchang"]</project-fonts>
...
[{"id":"b332DlJJ4","displayName":"Cursor"}]
...
["Iconic","Hero","Feather","Meteor","Material","Basicons","Flowbite","Nonicons","Sargam","Lucide","Mage","Logos"]
</available-icon-sets>
...
<additional-context>
{"userName":"MA Calude","currentDate":"June 28, 2026","timeZone":"America/New_York"}
</additional-context>

<site-map>{"augiA20Il":"/","uAGwwKMNo":"/madrepunk","RFREtphhx":"/page"}</site-map>
